...
const Form = ({ setPost, post }) => {
    ...
    useEffect(() => {
        const getPost = async () => {
            if (id) { // Si hay id, obtener la publicación
                ...
                try {
                    const { data } = await axios(`https://deploy-back-potenciar-3rzpu5z84-potenciarsolidarios-projects.vercel.app//posts/${id}`, config);
    ...
    const handleUpdate = async (id, info) => {
        const { data } = await axios.put(`https://deploy-back-potenciar-3rzpu5z84-potenciarsolidarios-projects.vercel.app//posts/${id}`, { ...info }, {
            headers: {
