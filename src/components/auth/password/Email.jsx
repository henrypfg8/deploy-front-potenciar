  ...
  const handleSendEmail = async (email) => {
    try {
      const { data } = await axios.put(
        "https://potenciar-solidario.onrender.com/forgotpassword",
        { email }
      ); // se envia el email
    ...
    <div>
      {successEmail ? ( // En caso de que el email si exista se mostrará, un mensaje
        <div className={Styles.email__container}>
          ...
          <button
            className={Styles.email__button__ok}
            onClick={() => {
        ...
        <div className={Styles.email__container}>
          <form method="post" onSubmit={onsubmit}>
            ...
            {errorEmail && (
              <p className={Styles.email__error}>
                Tu correo aún no esta registrada
              </p>
            )}
            <div style={{ width: "400px" }}>
              ...
              {errors?.email?.type === "required" && (
                <p style={{ color: "#ff0000" }}>Este campo es requerido</p>
              )}
            ...
            <button className={Styles.email__button} type="submit">
              Enviar
            </button>
          </form>
        </div>
